...
$.fn.ajaxSubmit = function(options) {
	...
	// 许文轩增加 序列化的时候trim
		...
		function param(a, traditional) {
			var s = [], add = function(key, value) {
				...
				s[s.length] = encodeURIComponent(key) + "=" + encodeURIComponent($.trim(value));
			};
	...
	//update by duanp 20120830 for validate error filepath
	if(fileInputs){
		var ins = $('input:file', this);
		var regexp = /^[a-zA-Z]:|\\/;
		for(var ii = 0 ; ii < ins.length ; ii++){
			var str = ins[ii].value;
			if(str && str != null && str != ""){
	　　			var r = regexp.test(str);
	　　			if(!r){
	　　				if($.alert){
	　　					$.alert("文件不存在或者文件大小为零");
					}else{alert("文件不存在或者文件大小为零");}	
